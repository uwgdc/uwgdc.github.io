...
## The _State_ of Reactivity _(ahaha)_
...
To demonstrate reactivity as a concept, we're going to introduce two types: `Value`s and `Computed`s. Values store manually-set values, while `Computed`s automatically react to `Value`s. _(or other `Computed`s!)_
...
Okay, reactive logic is nice and all, but it's not _real_. How do I get it onto the screen?

Let's expand our lever example. Instead of manually placing these levers, we can actually have our defined `RequiredLevers` _place them for us_. For this, we're going to use a new type: `ForPairs`.
